...
import config from '../config.vue' //conexion
  export default {
    ...
    cargarModal () { // Función que recarga los datos de la Tabla mediante request a la API REST
      ...
      cargarSelectSala () {
        const AuthStr = 'Bearer '.concat(this.$store.state.auth.accessToken)
        axios.get(config.API_LOCATION + `/skynet/sala/piso2`, { headers: { Authorization: AuthStr } }) // petición GET a Categoria para traer a todos los objetos "categoria"que contengan como tipo "insumo"
          .then((response) => {
